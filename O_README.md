...
-e "JENKINS_AGENT_SSH_PUBKEY=ssh-rsa  smag@smag-IdeaPad" \
jenkins/ssh-agent:alpine`
...
apk add nodejs npm firefox
cd /produits
chown jenkins:jenkins simulateur
exit`

```
docker cp isak0-bot/plugins agent3:/produits/simulateur/
docker cp isak0-bot/scenarios agent3:/produits/simulateur/
```
...
npm init -y
npm install selenium-webdriver assert mocha geckodriver free-proxy pg --save

!!!! non utilisation du container selenium-ff (voir le chapitre suivant dans readme)
#  npm install --save chromedriver  @sitespeed.io/edgedriver # if necessaire
...
- selenium
`docker run --name selenium-ff -d -p 4444:4444 -p 7900:7900 -e SE_NODE_MAX_SESSIONS=10 -e SE_NODE_OVERRIDE_MAX_SESSIONS=true --shm-size="2g" selenium/standalone-firefox`
